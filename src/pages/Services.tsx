...
const serviceCategories = [
  ...
  },
  {
    ...
    description: "Evidence-based cosmetic treatments focused on natural results and skin health.",
    services: [
      "PRP (Platelet Rich Plasma) Therapy",
      "GFC (Growth Factor Concentrate) Therapy",
      "Chemical Peels",
      "Acne & Acne Marks Treatment",
        ...
        <div className="container-custom">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-foreground mb-6">
              Comprehensive{" "}
              <span className="text-gradient">Dermatological Care</span>
            </h1>
          ...
          {serviceCategories.map((category, index) => (
            ...
            >
              <div className={index % 2 === 1 ? "lg:order-2" : ""}>
                ...
                {category.note && (
                  <div className="flex items-start gap-3 p-4 rounded-xl bg-secondary border border-primary/20 mb-6">
                    <AlertCircle className="w-5 h-5 text-primary flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-foreground">{category.note}</p>
                  </div>
                )}
              </div>
              <div className={`p-8 rounded-3xl bg-card border border-border ${index % 2 === 1 ? "lg:order-1" : ""}`}>
